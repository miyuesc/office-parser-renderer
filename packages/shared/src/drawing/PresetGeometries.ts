...
const ShapeRegistry: Record<string, ShapeGenerator> = {
  ...
  [ST_ShapeType.round2DiagRect]: (w, h, adj) => {
    const val = adj?.['val'] ?? 16667;
    const r = Math.min(w, h) * (val / 100000);
    return `M ${r} 0 L ${w} 0 L ${w} ${h - r} A ${r} ${r} 0 0 1 ${
      w - r
    } ${h} L 0 ${h} L 0 ${r} A ${r} ${r} 0 0 1 ${r} 0 Z`;
  },
  ...
  [ST_ShapeType.frame]: (w, h, adj) => {
    ...
    // 外框 + 内框 (使用 evenodd 填充)
    return `M 0 0 L ${w} 0 L ${w} ${h} L 0 ${h} Z M ${thick} ${thick} L ${thick} ${h - thick} L ${w - thick} ${
      h - thick
    } L ${w - thick} ${thick} Z`;
  },
  ...
  [ST_ShapeType.teardrop]: (w, h, adj) => {
    ...
    return `M ${cx} ${peakY} Q ${w} ${h * 0.2} ${w} ${h - r} A ${r} ${r} 0 1 1 0 ${h - r} Q 0 ${
      h * 0.2
    } ${cx} ${peakY} Z`;
  },
  ...
  [ST_ShapeType.heart]: (w, h) => {
    // 心形使用贝塞尔曲线
    const cx = w / 2;
    return `M ${cx} ${h * 0.25} C ${w * 0.1} ${-h * 0.1} ${-w * 0.2} ${h * 0.4} ${cx} ${h} C ${w * 1.2} ${h * 0.4} ${
      w * 0.9
    } ${-h * 0.1} ${cx} ${h * 0.25} Z`;
  },
  ...
  [ST_ShapeType.cloud]: (w, h) => {
    ...
    const r2 = w * 0.2;
    const r3 = w * 0.15;
    return `M ${w * 0.2} ${h * 0.7} A ${r1} ${r1} 0 1 1 ${w * 0.45} ${h * 0.3} A ${r2} ${r2} 0 1 1 ${w * 0.75} ${
      h * 0.35
    } A ${r3} ${r3} 0 1 1 ${w * 0.9} ${h * 0.6} A ${r2} ${r2} 0 1 1 ${w * 0.7} ${h * 0.85} L ${w * 0.3} ${
      h * 0.85
    } A ${r2} ${r2} 0 1 1 ${w * 0.2} ${h * 0.7} Z`;
  },
  ...
  [ST_ShapeType.plus]: (w, h, adj) => {
    ...
    const cy = h / 2;
    const hw = arm / 2;
    return `M ${cx - hw} 0 L ${cx + hw} 0 L ${cx + hw} ${cy - hw} L ${w} ${cy - hw} L ${w} ${cy + hw} L ${cx + hw} ${
      cy + hw
    } L ${cx + hw} ${h} L ${cx - hw} ${h} L ${cx - hw} ${cy + hw} L 0 ${cy + hw} L 0 ${cy - hw} L ${cx - hw} ${
      cy - hw
    } Z`;
  },
  ...
  [ST_ShapeType.noSmoking]: (w, h, adj) => {
    ...
    // 禁止符号：圆环 + 斜线
    return (
      GeoUtils.ellipse(cx, cy, rx, ry) +
      ' ' +
      GeoUtils.ellipse(cx, cy, rx - thick, ry - thick) +
      ` M ${w * 0.15} ${h * 0.85} L ${w * 0.85} ${h * 0.15}`
    );
  },
  ...
  [ST_ShapeType.blockArc]: (w, h, adj) => {
    ...
    return `M ${sx} ${sy} A ${rx} ${ry} 0 1 1 ${ex} ${ey} L ${cx + (rx - thick) * Math.cos(endAngle)} ${
      cy + (ry - thick) * Math.sin(endAngle)
    } A ${rx - thick} ${ry - thick} 0 1 0 ${cx + (rx - thick) * Math.cos(startAngle)} ${
      cy + (ry - thick) * Math.sin(startAngle)
    } Z`;
  },
  ...
  [ST_ShapeType.mathNotEqual]: (w, h, adj) => {
    // OOXML 规范参数
    const adj1 = adj?.['adj1'] ?? 23520; // 线宽比例 (~23.5%)
    // const adj2 = adj?.['adj2'] ?? 4210;   // 斜线角度偏移 (Unused)
    const adj3 = adj?.['adj3'] ?? 11760; // 横线间隔比例 (~12%)

    // 计算基础尺寸
    const lineW = h * (adj1 / 100000); // 线宽基于高度
    const gap = h * (adj3 / 100000); // 间隔基于高度

    // 横线位置（垂直居中，两条线之间有间隔）
    const cy = h / 2;
    const y1Top = cy - gap / 2 - lineW / 2;
    const y1Bot = cy - gap / 2 + lineW / 2;
    const y2Top = cy + gap / 2 - lineW / 2;
    const y2Bot = cy + gap / 2 + lineW / 2;
    ...
    // 斜线：从左下到右上穿过等号
    // 斜线应该从底部左侧延伸到顶部右侧（经典的 "≠" 样式）
    // const slashRad = (slashAngle * Math.PI) / 180; (Unused)

    // 斜线的起点和终点（从左下到右上）
    const slashStartX = w * 0.25;
    const slashEndX = w * 0.75;
    const slashStartY = h * 0.85;
    const slashEndY = h * 0.15;

    // 斜线需要有宽度，构造为一个斜向矩形
    const halfW = lineW * 0.45; // 斜线宽度稍细一些
    // 计算斜线的方向向量并归一化
    ...
    // 斜线的四个角点
    const p1x = slashStartX + perpX;
    const p1y = slashStartY + perpY;
    const p2x = slashStartX - perpX;
    const p2y = slashStartY - perpY;
    const p3x = slashEndX - perpX;
    const p3y = slashEndY - perpY;
    const p4x = slashEndX + perpX;
    const p4y = slashEndY + perpY;
   ...
   * adj1: 线宽比例，默认 23520
   */
  [ST_ShapeType.mathMultiply]: (w, h, _adj) => {
    // 两条交叉斜线
