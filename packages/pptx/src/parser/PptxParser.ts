...
import { PptxDocument, Slide, SlideElement } from '../types.ts';
...
export class PptxParser {
    ...
    private parseSlide(xml: string, rels: Record<string, string>, loadImage: (target: string) => string): Slide {
        ...
        if (spTree) {
             ...
             for (const child of children) {
                 ...
                 if (el.tagName.endsWith('sp')) { // Shape
                      const sp = this.parseShape(el);
                      if (sp) elements.push(sp);
                 } else if (el.tagName.endsWith('pic')) { // Picture
    ...
    private parsePicture(node: Element, rels: Record<string, string>, loadImage: (target: string) => string): import('../types').Picture | null {
