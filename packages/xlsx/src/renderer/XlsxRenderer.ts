...
import { generatePresetPath, resolveThemeColor } from '@ai-space/shared';
...
export class XlsxRenderer {
  ...
  private renderSheet(container: HTMLElement) {
    ...
    // 扩展 maxCol 和 maxRow 以包含图表、形状等插入元素的锚点范围
    ...
    // Rows - 确保生成足够的行以覆盖所有插入元素的锚点区域
    // const existingRowIndices = new Set(Object.keys(sheet.rows).map(Number)); (Unused)
  ...
  private renderDrawings(sheet: XlsxSheet, container: HTMLElement) {
    ...
    sheet.images?.forEach((img: OfficeImage) => {
      ...
      if (img.stroke) {
        ...
        // const color = img.stroke.color ? this.resolveColor(img.stroke.color) : 'black'; (Unused)
    ...
    sheet.connectors?.forEach((cxn: OfficeConnector) => {
      ...
      if (cxn.stroke?.gradient) {
        ...
        // const minX = Math.min(x1, x2, ...((cxn.adjustValues ? [/*TODO curve control points*/] : []) as any)); (Unused)
        ...
        color = (cxn.stroke?.color ? this.resolveColor(cxn.stroke.color) : undefined) || 'black';
      }
  ...
  private renderChart(
    ...
    if (chart.type === 'barChart') {
      ...
      this.renderPieChart(chart, g, rect, ctx);
    } else if (chart.type === 'lineChart') {
      this.renderLineChart(chart, g, rect, ctx);
    } else {
      // 占位符
  ...
  private renderBarChart(
    ...
    console.log(`[BarChart] numCats=${numCats}, numSeries=${numSeries}, plotW=${plotW}, categories=`, categories);
    if (numCats === 0) return;

    // 柱宽计算 - Office 风格
    // 每个分类占用的总宽度
    const catW = plotW / numCats;
    // 组间距：Office 默认让柱组占每个分类宽度的约 55%，两侧各留约 22.5% 作为间距
    const groupGapRatio = 0.45; // 45% 总间距（两侧各 22.5%）
  ...
  private renderPieChart(
    ...
    categories.forEach((cat, i) => {
      ...
      legendRect.setAttribute('width', '10');
      legendRect.setAttribute('height', '10');
      legendRect.setAttribute('fill', colors[i % colors.length]);
      container.appendChild(legendRect);
  ...
  private renderShapeInGroup(
    ...
    console.log(
      `Render Shape: [${shape.name}] Geometry: '${shape.geometry}' pathExists: ${!!shape.path} pathLength: ${
        shape.path?.length || 0
      }`,
      shape.adjustValues
    );
    ...
    if (shape.geometry === 'custom' && shape.path) {
      ...
      if (presetPath) {
        vector = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        // 使用 presetPath.path 访问路径字符串
        vector.setAttribute('d', presetPath.path);
        // 确保 fill 先绘制，stroke 后绘制（stroke 在外侧而非覆盖填充）
        vector.style.paintOrder = 'fill stroke';

        // 处理 noFill 标志（用于连接线等开放路径）
        if (presetPath.noFill) {
          vector.setAttribute('fill', 'none');
        }

        // 处理辅助描边路径（如 callout 指示线），需要创建额外的路径元素
        if (presetPath.strokePath) {
          ...
          // 描边路径的样式会在后续由 stroke 逻辑处理
          // @ts-ignore - 临时存储以便后续添加到容器
    ...
    if (vector._strokePathElement) {
      // @ts-ignore
      const strokePathEl = vector._strokePathElement as SVGPathElement;
      // 应用相同的描边样式
      if (strokeColor !== 'none') {
  ...
  private resolveFill(fill: any, ctx: any, _rect: any, asCss: boolean = false): string {
    if (!fill) return 'none';
  ...
  private resolveFilter(effects: any[], ctx: any): string | null {
    ...
    effects.forEach((eff: any) => {
      if (eff.type === 'outerShadow') {
        ...
        console.log(
          `[Shadow Offset] distPx=${distPx.toFixed(2)}, angle=${eff.dir}° -> dx=${dx.toFixed(2)}, dy=${dy.toFixed(2)}`
        );
