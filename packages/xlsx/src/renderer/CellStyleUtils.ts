...
import { XlsxWorkbook, XlsxCellXf, XlsxFont, XlsxFill, XlsxBorder, XlsxColor } from '../types';
...
export class CellStyleUtils {
    static getCss(workbook: XlsxWorkbook, styleIndex?: number): Partial<CSSStyleDeclaration> {
        ...
        if (xf.alignment) { // Relaxed check: ignore applyAlignment for robustness
            if (xf.alignment.horizontal) {
                switch (xf.alignment.horizontal) {
                    case 'center': css.textAlign = 'center'; break;
                    case 'right': css.textAlign = 'right'; break;
                    case 'justify': css.textAlign = 'justify'; break;
                    default: css.textAlign = 'left'; 
                }
            }
            if (xf.alignment.vertical) {
                 switch (xf.alignment.vertical) {
                    case 'top': css.verticalAlign = 'top'; break;
                    case 'center': css.verticalAlign = 'middle'; break;
                    case 'bottom': css.verticalAlign = 'bottom'; break;
                    default: css.verticalAlign = 'bottom';
                }
            }
        ...
        if (border) {
            const getBorderCss = (side: any) => {
                ...
                switch (side.style) {
                    case 'hair': width = '1px'; break; // thin
