...
export class ChartParser {
    ...
    private static parseSeriesData(chartNode: Element): ChartSeries[] {
        ...
        serNodes.forEach(ser => {
            ...
            const catPts = XmlUtils.queryAll(ser, 'c\\:cat c\\:strRef c\\:strCache c\\:pt');
            catPts.forEach(pt => {
            ...
            // 数值数据 (Y轴)
            const values: number[] = [];
            const valPts = XmlUtils.queryAll(ser, 'c\\:val c\\:numRef c\\:numCache c\\:pt');
            valPts.forEach(pt => {
                const v = XmlUtils.query(pt, 'c\\:v');
                if (v) {
