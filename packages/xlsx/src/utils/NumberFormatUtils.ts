...
export class NumberFormatUtils {
    ...
    private static formatDate(date: Date, fmt: string): string {
        ...
        // 4. Minutes vs Months
        // Strategy: Replace 'mm'/'m' based on context. 
        // If the format string contains 'h' (hours) or 's' (seconds) BEFORE the 'm', it's likely minutes?
        // Excel Logic: 'm' immediately after 'h' or 'hh' is minute. 'm' immediately before 's' or 'ss' is minute.
        
        // Replace Minutes first temporarily
        const minuteToken = 'MIN_TOKEN';
        // Regex lookbehind not fully supported in all JS? Use precise replace.
        // Replace h:mm -> h:MIN
        out = out.replace(/(h+)[:\s]+(m+)/gi, (match, h, mStr) => {
             return match.replace(mStr, mStr.length > 1 ? String(M).padStart(2,'0') : String(M));
        });
        // Replace mm:ss -> MIN:ss
        out = out.replace(/(m+)[:\s]+(s+)/gi, (match, mStr, s) => {
             return match.replace(mStr, mStr.length > 1 ? String(M).padStart(2,'0') : String(M));
        });
    ...
    private static formatPercent(value: number, fmt: string): string {
         ...
         const decimals = (fmt.split('.')[1] || '').length; // count chars after dot until %
         // "0.00%" -> 2 decimals
    ...
    private static formatFraction(value: number, fmt: string): string {
